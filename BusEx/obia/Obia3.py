...
Maquina de vetor suporte - linear: SVC(kernel="linear", C=0.025)
RBF SVM: SVC(gamma=2, C=1)
Processo Gaussiano: GaussianProcessClassifier(1.0 * RBF(1.0), warm_start=True)
Arvore de decisao: DecisionTreeClassifier(max_depth=5)
Floresta aleatoria: RandomForestClassifier(max_depth=5, n_estimators=10, max_features=1)
Vizinho mais proximo: KNeighborsClassifier(3)
AdaBoost : AdaBoostClassifier()
Naive Bayes: GaussianNB()
Discriminante Quadratico: QuadraticDiscriminantAnalysis
        '''
        teste = True
    ...
    def QGISgetRowColsBandsFiles2(self,img, TRAIN_DATA_PATH):
        rows, cols, n_bands = img.shape
        # files = TRAIN_DATA_PATH.values()
        # classes_labels = TRAIN_DATA_PATH.keys()
        classes_labels = TRAIN_DATA_PATH['labels']
        shapefiles = TRAIN_DATA_PATH['paths']
        # shapefiles = TRAIN_DATA_PATH.values()
        print(shapefiles)
        return rows, cols, n_bands, shapefiles, classes_labels
    ...
    def classificationValidation2(self,TEST_DATA_PATH, clf, classes_labels, rows, cols, geo_transform, proj):
        # shapefiles = [os.path.join(TEST_DATA_PATH, "%s.shp" % c) for c in classes_labels]
        shapefiles = TEST_DATA_PATH['paths']
        print('shapefiles')
        print(shapefiles)
        verification_pixels = self.vectors_to_raster(shapefiles, rows, cols, geo_transform, proj)
        for_verification = np.nonzero(verification_pixels)
        ...
        verification_labels = verification_pixels[for_verification]
        predicted_labels = clf[for_verification]
        # print('len(verification_labels)')
        # print(len(verification_labels))
        # print('len(predicted_labels)')
        # print(len(predicted_labels))
        # print('verification_labels')
        # print(verification_labels)
        # print('Uniques verification')
        # print(collections.Counter(verification_labels))
        # print('Unique 5')
        # print(str(verification_labels.count(5.0)))
        # print('Unique 6')
        # print(str(verification_labels.count(6.0)))
        # print('Unique 7')
        # print(str(verification_labels.count(7.0)))
        # print(verification_labels)
        # print(verification_labels)
        ...
        print("Parte 32")
        # In[49]:
        corrected_verification_labels, corrected_predicted_labels, erros_vetorizacao = self.correctVetorization(verification_labels , predicted_labels)
        cm = metrics.confusion_matrix(corrected_verification_labels, corrected_predicted_labels)
        # cm = metrics.confusion_matrix(verification_labels, predicted_labels)
        unique_verification_labels = list(set(verification_labels))
        print('erros_vetorizacao')
        print(erros_vetorizacao)
        # unique_predicted_labels = list(set(predicted_labels))
        # print('unique_verification_labels')
        # print(unique_verification_labels)
        # print('unique_predicted_labels')
        # print(unique_predicted_labels)
        return cm, corrected_verification_labels, corrected_predicted_labels, erros_vetorizacao

    def correctVetorization(self,verification_labels , predicted_labels):
        # Maior label de treinamento
        maxsize = max(predicted_labels)

        # labels de verificacao fora do conjunto de labels de treinamento
        elements_to_delete = []
        for idx, verification in enumerate(verification_labels):
            if verification > maxsize:
                elements_to_delete.append(idx)

        # Apagando elementos previstos erroneamente devido a vetorizacao
        corrected_verification_labels = verification_labels.tolist()
        corrected_predicted_labels = predicted_labels.tolist()
        for index in reversed(elements_to_delete):
            del corrected_verification_labels[index]
            del corrected_predicted_labels[index]

        erros_vetorizacao = len(elements_to_delete) + self.rows* self.cols - verification_labels.size
        return np.array(corrected_verification_labels), np.array(corrected_predicted_labels), erros_vetorizacao
    ...
    def QgisClassification(self):
        ...
        self.rows, self.cols, n_bands, shapefiles, self.classes_labels = self.QGISgetRowColsBandsFiles2(img, self.TRAIN_DATA_PATH)
        print('self.rows')
        print(self.rows)
        print('self.cols')
        print(self.cols)
        ...
        self.raster_to_tiff2(self.clf_image, self.CLASSIFICATED_IMAGE_PATH, self.rows, self.cols, self.geo_transform, self.proj)
    ...
    def validateClassification(self):
        ...
        cm, corrected_verification_labels, corrected_predicted_labels, erros_vetorizacao = self.classificationValidation2(self.TEST_DATA_PATH, self.clf_image,
        ...
        # classificationAccuracy, classificationReport = self.getClassicationMetrics(verification_labels,predicted_labels, self.classes_labels)
